...
export function findModule(host: Tree, generateDir: string,
                           moduleExt: string = MODULE_EXT, routingModuleExt: string = ROUTING_MODULE_EXT): Path {
...
export function findRoutingModule(host: Tree, generateDir: string,
                                  moduleExt: string = MODULE_EXT, routingModuleExt: string = ROUTING_MODULE_EXT) {
  ...
  while (dir) {
    const allMatches = dir.subfiles.filter(p => p.endsWith(moduleExt));
    const filteredMatches = allMatches.filter(p => p.endsWith(routingModuleExt));
    foundRoutingModule = foundRoutingModule || filteredMatches.length !== 0;
    if (filteredMatches.length === 1) {
      return join(dir.path, filteredMatches[0]);
    } else if (filteredMatches.length > 1) {
      throw new Error('More than one routing module matches');
    }
    dir = dir.parent;
  }
}
